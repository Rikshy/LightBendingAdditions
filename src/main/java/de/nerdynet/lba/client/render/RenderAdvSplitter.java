package de.nerdynet.lba.client.render;

import com.teamwizardry.refraction.api.Constants;
import de.nerdynet.lba.LightBendingAdditions;
import de.nerdynet.lba.client.ClientProxy;
...
public class RenderAdvSplitter extends TileEntitySpecialRenderer<TileAdvSplitter> {

	private IBakedModel modelArms, modelMirror;

	public RenderAdvSplitter() {
	...
	private void getBakedModels(TileAdvSplitter te) {
		IModel model = null;
		if (modelArms == null) {
			try {
		...
		if (modelMirror == null || te.hasInvChanged()) {
			try {
	...
	@Override
	public void render(TileAdvSplitter te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
		...
		Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelRenderer().renderModelBrightnessColor(modelMirror, 1.0F, 1, 1, 1);
